...
# *开发人: Xiaofeng XU
# *开发日: 2021-06-22
# *修改记录: 1.bug-fix：insert overwrite 不适用这里，
# *********************************************************************/
...
# init_target_table
#target_table = 'cdm_{0}_tp_ts_all_tmp'.format(brand.lower())
#hc.sql('select mobile,action_time,touchpoint_id,brand,pt,source from marketing_modeling.cdm_{0}_tp_ts_all_i where pt={1}'.format(brand.lower(),pt2)).createOrReplaceTempView(target_table)
...
for df in df_col:
    ...
    hc.sql('insert overwrite table marketing_modeling.cdm_{0}_tp_ts_all_i PARTITION (pt,source) select mobile,action_time,touchpoint_id,brand,pt,source from final_df'.format(brand.lower()))
...
# hc.sql('insert overwrite table marketing_modeling.cdm_{0}_tp_ts_all_i PARTITION (pt,source) select mobile,action_time,touchpoint_id,brand,pt,source from {1}'.format(brand.lower(),target_table))
