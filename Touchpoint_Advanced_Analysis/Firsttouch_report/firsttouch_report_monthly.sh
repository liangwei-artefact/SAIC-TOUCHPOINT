...
final_df AS (
    ...
    SELECT
        grouped_profile_df.fir_contact_month,
        ...
        nvl(instore_df.instore_vol / grouped_profile_df.cust_vol, 0) AS instore_rate -- 到店率
    FROM grouped_profile_df
    LEFT JOIN instore_df
	ON
        grouped_profile_df.fir_contact_month = instore_df.action_month AND
    ...
    LEFT JOIN trial_df
	ON
        grouped_profile_df.fir_contact_month = trial_df.action_month AND
	...
	ON
        grouped_profile_df.fir_contact_month = consume_df.action_month AND
	...
	ON
        grouped_profile_df.fir_contact_month = deliver_df.action_month AND
